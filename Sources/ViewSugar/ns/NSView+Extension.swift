...
extension NSView {
   ...
   var mouseX: CGFloat { MouseUtils.point(self).x } /*UNTESTED*/
   var mouseY: CGFloat { MouseUtils.point(self).y } /*UNTESTED*/
   ...
   func getSubViewAt(_ i: Int) -> NSView? { NSViewParser.getSubViewAt(self, i) }
   var numSubViews: Int { subviews.count }/*convenience*/
   /**
    * New
    */
   func first<T>(ofKind: T.Type? = nil) -> T? {
      NSViewParser.first(view: self, ofKind: ofKind)
   }
}
...
extension View {
   ...
   func contains<T: View>(_ view: T) -> Bool {
      NSViewAsserter.contains(self, view)
   }
}
#endif
