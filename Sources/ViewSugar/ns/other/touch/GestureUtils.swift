...
class GestureUtils {
   ...
   static func twoFingersTouches(_ view: NSView, _ event: NSEvent) -> [String: NSTouch]? {
      var twoFingersTouches: [String: NSTouch]? // NSMutualDictionary was used before and didn't require casting id to string, revert if sideeffects manifest
    ...
    * - Fixme: ⚠️️ also make up and down swipe detectors, and do more research into how this could be done easier. Maybe you even have some clues in the notes about gestures etc.
    * Conceptually:
   ...
   static func swipe(_ view: NSView, _ event: NSEvent, _ twoFingersTouches: [String: NSTouch]?) -> SwipeType {
      ...
      if !endingTouches.isEmpty && twoFingersTouches != nil {
