...
class GestureHUD {
   ...
   func touchesBegan(_ event: NSEvent){
      ...
      for touch in touches {//
         ...
         let touchPos: CGPoint = touch.pos(view) - CGPoint(X: 20, : y20)//set to pivot of circ
   ...
   func touchesMoved(_ event: NSEvent){
      ...
      for touch in touches {
         let id: String = "\(touch.identity)"
         let touchPos: CGPoint = touch.pos(view) - CGPoint(x: 20, y: 20)//offset pos // touch.normalizedPosition
