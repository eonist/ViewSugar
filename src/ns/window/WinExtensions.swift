...
extension NSWindow {
   ...
   var flippedScreenPosition: CGPoint { return .init(x: NSEvent.mouseLocation.x, y: abs(NSEvent.mouseLocation.y + -NSScreen.main!.visibleFrame.height))/*flip the y coordianate:*/ }/*Returns the topLeft postion of the win in relation to the screen*/
   var topLeft: CGPoint { return .init(x: frame.origin.x, y: NSScreen.main!.visibleFrame.height - self.frame.origin.y - self.frame.size.height) }/*returns the topLeft pos (in a flipped coordinate space aka 0,0 coordinate space)*/
